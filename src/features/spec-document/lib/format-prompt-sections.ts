import { IAcceptanceCriteria, IEdgeCase, IOpenQuestion, IComment } from 'shared/types';

/**
 * Format user stories as a numbered list
 */
export function formatUserStories(stories: string[]): string {
  if (stories.length === 0) return '(None specified)';
 ...
 */
export function formatAcceptanceCriteria(criteria: IAcceptanceCriteria[]): string {
  if (criteria.length === 0) return '(None specified)';

  return criteria.map((item, index) => `${index + 1}. ${item.description}`).join('\n');
}
...
export function formatEdgeCases(cases: IEdgeCase[]): string {
  if (cases.length === 0) return '(None specified)';

  return cases
    .map(edgeCase => `- When ${edgeCase.scenario}: ${edgeCase.expectedBehavior}`)
    .join('\n');
}
 ...
 */
export function formatArrayAsBulletList(items: string[], prefix: string = '-'): string {
