...
function TeamContent() {
  ...
  const [inviteRole, setInviteRole] = useState<'developer' | 'founder'>('developer');

  // Mock team members - filter by current user's organization
  const teamMembers = mockUsers.filter(u => u.organizationId === currentUser?.organizationId);
    ...
    <main className="p-6">
          ...
          <Card className="mb-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  value={inviteRole}
                  onChange={e => setInviteRole(e.target.value as 'developer' | 'founder')}
              ...
              <div className="rounded-lg border border-blue-100 bg-blue-50 p-3 text-sm text-blue-800 dark:border-blue-900/50 dark:bg-blue-900/20 dark:text-blue-200">
                <strong>Note:</strong> Invitations expire after 7 days. The invitee will receive an
                email with a link to join your organization.
              </div>
            </div>
          </Card>
        )}
        ...
        {teamMembers.length === 0 && (
          <EmptyState
            ...
            action={<Button onClick={() => setShowInviteForm(true)}>Invite Member</Button>}
          />
        )}
    </main>
  );
